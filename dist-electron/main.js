...
import electron, { app as app$1, protocol, ipcMain as ipcMain$1, screen, BrowserWindow, Menu, dialog } from "electron";
...
function countKeys$1(schema) {
  ...
  for (const key in schema) {
    ...
    if (typeof schema[key] == "object") {
      (0, util_1$S.eachItem)(schema[key], (sch) => count += countKeys$1(sch));
    }
    if (count === Infinity)
      return Infinity;
  }
...
(function(exports) {
  ...
  function requiredOptions(o) {
    var _a2, _b2, _c2, _d2, _e2, _f2, _g2, _h2, _j2, _k2, _l2, _m2, _o2, _p2, _q2, _r2, _s2, _t2, _u2, _v2, _w2, _x2, _y2, _z2, _0;
    const s = o.strict;
    ...
    return {
      ...
      addUsedSchema: (_w2 = o.addUsedSchema) !== null && _w2 !== void 0 ? _w2 : true,
      validateSchema: (_x2 = o.validateSchema) !== null && _x2 !== void 0 ? _x2 : true,
      validateFormats: (_y2 = o.validateFormats) !== null && _y2 !== void 0 ? _y2 : true,
      unicodeRegExp: (_z2 = o.unicodeRegExp) !== null && _z2 !== void 0 ? _z2 : true,
...
const def$T = {
  ...
  code(cxt) {
    ...
    if (useLoop || $data) {
      ...
      valid2 = (0, codegen_1$L.or)(...schema.map((_x2, i) => equalCode(vSchema, i)));
    }
    cxt.pass(valid2);
    function loopEnum() {
      gen.assign(valid2, false);
  ...
  code(cxt) {
    ...
    if (useLoop || $data) {
      ...
      if (!Array.isArray(schema))
        throw new Error("ajv implementation error");
      const vSchema = gen.const("vSchema", schemaCode);
      valid2 = (0, codegen_1$b.or)(...schema.map((_x2, i) => equalCode(vSchema, i)));
    }
    cxt.pass(valid2);
    function loopEnum() {
      gen.assign(valid2, false);
...
_b = entityKind;
class ColumnBuilder {
  constructor(name, dataType2, columnType) {
    __publicField(this, "config");
    /**
...
class SQLiteBaseIntegerBuilder extends (_sa = SQLiteColumnBuilder, _ra = entityKind, _sa) {
  constructor(name, dataType2, columnType) {
    super(name, dataType2, columnType);
...
class SQLiteTable extends (_hb = Table, _gb = entityKind, _fb = Table.Symbol.Columns, _eb = InlineForeignKeys, _db = Table.Symbol.ExtraConfigBuilder, _hb) {
  constructor() {
    super(...arguments);
    /** @internal */
...
function getDb() {
  ...
  const dbPath = path.join(app$1.getPath("userData"), "seedscreen.db");
...
function addSong(input) {
  const row = getDb().insert(songs).values({
...
function updateSong(id2, input) {
  const row = getDb().update(songs).set({
    title: input.title.trim(),
    author: (input.author || "").trim(),
    language: input.language || "es",
    slides: JSON.stringify(input.slides || []),
    updatedAt: sql`(datetime('now'))`
  }).where(eq(songs.id, id2)).returning().get();
  return row ? parseSong(row) : null;
}
function deleteSong(id2) {
...
const VIRTUAL_RE = /(vethernet|virtualbox|vmware|hyper-?v|wsl|default switch|loopback|docker|tailscale|zerotier|utun|llw|awdl|bridge|ppp|tun|tap)/i;
function candidates() {
  const out = [];
...
function score(i) {
  ...
  if (i.address.startsWith("192.168.56.")) s -= 60;
  if (i.address.startsWith("192.168.")) s += 30;
  else if (i.address.startsWith("10.")) s += 25;
  else if (/^172\.(1[6-9]|2\d|3[01])\./.test(i.address)) s += 10;
  return s;
}
function localIp() {
...
function directedBroadcast(address, netmask) {
  ...
  return a.map((oct, idx) => oct & m[idx] | ~m[idx] & 255).join(".");
}
function broadcastAddresses() {
  const set = /* @__PURE__ */ new Set(["255.255.255.255"]);
...
function getBible() {
  ...
  const devPath = path.join(__dirname$1, "../resources/bible.json");
...
function createOutputWindow(displayId) {
  ...
  outputWin.on("closed", () => {
    outputWin = null;
    win == null ? void 0 : win.webContents.send("output-window-closed");
  });
}
      ...
      submenu: [
        {
          ...
          click: () => win == null ? void 0 : win.webContents.send("open-settings")
        },
    ...
    submenu: [
      {
        ...
        click: () => win == null ? void 0 : win.webContents.send("menu-new-song")
      },
      {
        label: "New song with AI",
        accelerator: "CmdOrCtrl+Shift+I",
        click: () => win == null ? void 0 : win.webContents.send("menu-new-song-ai")
      }
    ]
  },
  {
    ...
    submenu: [
      {
        ...
        click: () => win == null ? void 0 : win.webContents.send("menu-toggle-output")
      },
      { type: "separator" },
      {
...
ipcMain$1.handle(
  "backgrounds:add",
  (_event, bg) => {
    const item = { id: randomUUID(), ...bg };
    store.set("backgrounds", [...store.get("backgrounds"), item]);
    return item;
  }
);
ipcMain$1.handle("backgrounds:delete", (_event, id2) => {
  store.set(
    "backgrounds",
...
ipcMain$1.handle("output:show-youtube", (_event, videoId) => {
  outputWin == null ? void 0 : outputWin.webContents.send("show-youtube", videoId);
  return true;
});
...
function withUrl(rows) {
  return rows.map((row) => ({ ...row, url: mediaUrlFor(row.filePath) }));
}
ipcMain$1.handle("media:get-all", () => withUrl(getMedia()));
...
ipcMain$1.handle("bible:get-chapter", (_event, bookId, chapterNum) => {
  ...
  if (!chapter) return [];
  return chapter.items.filter((it) => it.type === "verse" && it.verse_numbers.length > 0).map((it) => ({ v: it.verse_numbers[0], t: it.lines.join(" ") }));
});
ipcMain$1.handle("bible:search", (_event, query) => {
  ...
  for (const book of getBible().books) {
    ...
    for (let ci = 0; ci < chapters.length; ci++) {
      for (const it of chapters[ci].items) {
        ...
        const text2 = it.lines.join(" ");
        if (text2.toLowerCase().includes(q)) {
          results.push({ bookName: book.name, abbr, chapter: ci + 1, verse: it.verse_numbers[0], text: text2 });
