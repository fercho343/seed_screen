...
const bibleCache = /* @__PURE__ */ new Map();
function getBibleLang(lang) {
  const cached = bibleCache.get(lang);
  if (cached) return cached;
  const packagedPath = path.join(process.resourcesPath, "bible", `${lang}.json`);
  const devPath = path.join(__dirname$1, "../resources/bible", `${lang}.json`);
...
ipcMain$1.handle("bible:search", (_event, query, lang = "es") => {
  ...
  for (const v of data.verses) {
    ...
    const abbr = ((_a2 = data.books[v.book - 1]) == null ? void 0 : _a2.abbr) ?? "";
